...
# echo "SEGMENT CONTRAST 0.1%"
# python3 inference_vis.py --dataset-name SemanticKITTI --log-dir ./checkpoint/downstream_task/percent_0.1/{DIR_NAME} --data-dir ./Datasets/SemanticKITTI/ --sparse-model MinkUNet --sparse-resolution 0.05 --batch-size 1 --checkpoint tarl --use-cuda --best epoch299 --use-intensity

#!/bin/bash

BASE_DIR="./checkpoint/downstream_task/percent_0.1"

for dir in "$BASE_DIR"/*; do
    if [ -d "$dir" ]; then
        DIR_NAME=$(basename "$dir")
        echo "Running inference for checkpoint: $DIR_NAME"
        CUDA_VISIBLE_DEVICES=0 python3 inference_vis.py \
