...
CUDA_VISIBLE_DEVICES=${gpu} python3 inference_vis.py --dataset-name SemanticKITTI --log-dir checkpoint/downstream_task/${bs_dir}percent_0.1/${ckpt_dir}/ --data-dir ./Datasets/SemanticKITTI/ --sparse-model MinkUNet --sparse-resolution 0.05 --batch-size 4 --checkpoint tarl --use-cuda --best epoch299 --use-intensity

echo "SEGMENT CONTRAST 1%"
CUDA_VISIBLE_DEVICES=${gpu} python3 inference_vis.py --dataset-name SemanticKITTI --log-dir ./checkpoint/downstream_task/${bs_dir}percent_1.0/${ckpt_dir}/ --data-dir ./Datasets/SemanticKITTI/ --sparse-model MinkUNet --sparse-resolution 0.05 --batch-size 4 --checkpoint tarl --use-cuda --best epoch119 --use-intensity 
